...
export function initDesignStudio() {
    ...
    function up(event) {
        const wasDragging = !!studio.drag;
        const wasPinching = !!studio.pinch;
        studio.pointers.delete(event.pointerId);
        if (studio.pointers.size < 2) studio.pinch = null;
        studio.drag = null;
        // Pointer hərəkətinin son vəziyyətini tarixçəyə ayrıca yazırıq.
        // Beləliklə undo/redo və Design Studio qaralaması son real mövqeni saxlayır.
        if (wasDragging || wasPinching) {
            queueHistory(wasPinching ? 'İki barmaq ilə ölçü' : 'Element hərəkəti');
            saveLayout({ silent: true, persistDraft: true });
        }
    }
    ...
    function addTextElement() {
        const input = el('customText');
        const current = String(input?.value || '').trim();
        if (!current) {
            if (input) {
                input.value = 'Sənin yazın';
                input.focus();
                input.select();
            }
        }
        studio.elements.text.visible = true;
        studio.selected = 'text';
        syncFormToState({ history: false });
        pushHistory('Mətn əlavə edildi');
        draw();
    }

    function addImageElement() {
        const input = el('customImage');
        if (!input) return;
        input.click();
    }
    ...
    Object.assign(ctx, {
        ...
        addDesignText: addTextElement,
        addDesignImage: addImageElement,
