...
export function initCustomization() {
    ...
    function customizationSignature(c){
        const src = c && typeof c === "object" && !Array.isArray(c) ? c : {};
        const layout = src.layout && typeof src.layout === "object" ? src.layout : null;
        const normalized = {
            text: String(src.text || "").trim(),
            color: String(src.color || "").trim(),
            size: String(src.size || "").trim(),
            sizeValue: String(src.sizeValue || "").trim(),
            font: String(src.font || "").trim(),
            imageName: String(src.imageName || "").trim(),
            imagePath: String(src.imagePath || "").trim(),
            imageType: String(src.imageType || "").trim(),
            imageSize: Number(src.imageSize || 0) || 0,
        ...
        try{
            ...
            if(s.customEditingLineId){
                ...
                if(item){
                    ...
                    item.customization={...customization, layout: s.customDesignLayout || null};
                    ctx.saveCart();
                    ctx.clearDesignDraft?.(p.id);
                    ctx.renderCart(); ctx.closeCustomModal();
            ...
            customization.layout = s.customDesignLayout || customization.layout || null;
            ...
            if(existing){ existing.qty=Math.min(99, Number(existing.qty||0)+1); }
            else { s.cart.push({id:p.id,qty:1,lineId:ctx.makeClientId(`line-${p.id}`),customization}); }
            ctx.saveCart();
            ctx.clearDesignDraft?.(p.id);
            ctx.closeCustomModal(); ctx.openCart();
