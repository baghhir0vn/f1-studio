...
const PRODUCT_COLUMNS = 'id,name,price,cat,image,images,emoji,badge,material,size,production_time,stock,stock_quantity,description,tags,customizable,sort_order,is_active,is_featured,updated_at';
...
export function mapProductRow(p) {
    ...
    return {
        ...
        sortOrder: Number.isFinite(Number(p.sort_order)) ? Number(p.sort_order) : 0,
...
const writePayload = x => {
    ...
    return {
        ...
        is_featured: x.isFeatured === true,
        sort_order: Number.isFinite(Number(x.sortOrder)) ? Math.max(0, Math.min(1000000, Math.floor(Number(x.sortOrder)))) : 0
    };
};
...
export const productService = {
    async list(options = {}) {
        ...
        let query = sb.from('products').select(PRODUCT_COLUMNS).order('sort_order').order('id');
    ...
    async create(x) {
        ...
        if (error) throw error;
        const normalized = mapProductRow(data);
        if (!Number(normalized.sortOrder)) {
            const { data: maxRows, error: maxError } = await sb.from('products').select('sort_order').order('sort_order', { ascending: false }).limit(1);
            if (!maxError) {
                const max = Number(maxRows?.[0]?.sort_order) || 0;
                const next = Math.max(max + 10, Number(normalized.id) * 10);
                const { data: updated, error: updateError } = await sb.from('products').update({ sort_order: next }).eq('id', normalized.id).select(PRODUCT_COLUMNS).single();
                if (!updateError && updated) return mapProductRow(updated);
            }
        }
        return normalized;
    },
    ...
    async updateFlags(id, flags = {}) {
        if (!Number.isInteger(Number(id)) || Number(id) <= 0) throw new Error('INVALID_PRODUCT');
        const payload = {};
        if (Object.prototype.hasOwnProperty.call(flags, 'isActive')) payload.is_active = flags.isActive === true;
        if (Object.prototype.hasOwnProperty.call(flags, 'isFeatured')) payload.is_featured = flags.isFeatured === true;
        if (!Object.keys(payload).length) throw new Error('INVALID_PRODUCT');
        const { data, error } = await sb.from('products')
            .update({ ...payload, updated_at: new Date().toISOString() })
            .eq('id', id).select(PRODUCT_COLUMNS).single();
        if (error) throw error;
    ...
    async reorder(orderIds = []) {
        if (!Array.isArray(orderIds) || !orderIds.length || orderIds.length > 1000) throw new Error('INVALID_PRODUCT_ORDER');
        const ids = orderIds.map(v => Number(v));
        if (ids.some(v => !Number.isInteger(v) || v <= 0) || new Set(ids).size !== ids.length) throw new Error('INVALID_PRODUCT_ORDER');
        const { data, error } = await sb.rpc('admin_reorder_products', { p_order_ids: ids });
        if (error) throw error;
        return data && typeof data === 'object' ? data : { ok: true, count: ids.length };
    },
    async remove(id) {
        if (!Number.isInteger(Number(id)) || Number(id) <= 0) throw new Error('INVALID_PRODUCT');
