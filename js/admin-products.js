...
async function loadAdminProducts(){
    ...
    function renderAdminProducts(){
        ...
        body.innerHTML=list.map(p=>{
            ...
            return `<tr><td><div class="admin-product-thumb">${safeResourceUrl(p.image)?`<img loading="lazy" decoding="async" src="${escapeHTML(safeResourceUrl(p.image))}" alt="" style="width:100%;height:100%;object-fit:cover" data-fallback-emoji="${escapeHTML(p.emoji||"📦")}">`:escapeHTML(p.emoji||"📦")}</div></td><td><b>${escapeHTML(p.name)}</b><div class="admin-muted">${escapeHTML(p.badge||"")}${galleryCount?` · ${galleryCount} şəkil`:""}</div></td><td>${escapeHTML(p.cat||"")}</td><td>${money(p.price)}</td><td>${stockLabel}</td><td><div class="admin-actions"><button data-action="editAdminProduct" data-action-args='[${Number(p.id)}]'>✏️ Dəyiş</button><button class="danger" data-action="deleteAdminProduct" data-action-args='[${Number(p.id)}]'>🗑 Sil</button></div></td></tr>`;
        }).join("");
