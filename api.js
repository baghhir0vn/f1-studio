...
export function createApi(deps = {}) {
    ...
    return async function api(path, options = {}) {
        ...
        if (path === '/api/admin/products/reorder' && method === 'POST') {
            requireAdmin(isAdminUser);
            return productService.reorder(body?.order_ids || []);
        }
        ...
        if (adminMatch) {
            ...
            if (type === 'products') {
                if (method === 'GET') return { products: await productService.list({ includeInactive: true }) };
                if (method === 'POST') return { product: await productService.create(body) };
                if (method === 'PATCH' && id) return { product: await productService.updateFlags(id, body) };
            ...
            if (type === 'orders') {
                ...
                if (method === 'PATCH' && id) {
                    if (Object.prototype.hasOwnProperty.call(body || {}, 'admin_note')) return { order: await orderService.setAdminNote(id, body.admin_note) };
                    return { order: await orderService.setStatus(id, body.status) };
                }
            }
            if (type === 'reviews') {
