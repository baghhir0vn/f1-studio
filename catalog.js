...
export function initCatalog() {
    ...
    function render() {
        ...
        if (s.viewingFavs) {
            ...
            if (!s.showAllProducts && !q.trim() && !s.activeCat) list = list.slice(0, 5);
        }
    ...
    function drawGrid(list) {
        ...
        list.forEach(p => {
            ...
            const outOfStock=p.stockQuantity!=null && Number(p.stockQuantity)<=0;
            const addBtn=document.createElement("button"); addBtn.className="add"; addBtn.type="button"; addBtn.disabled=outOfStock; addBtn.innerHTML=outOfStock?"—":(p.customizable?"＋":"<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 4h2l1.6 10.2a2 2 0 0 0 2 1.8h7.9a2 2 0 0 0 1.9-1.5L20 8H6"></path><circle cx="9" cy="19" r="1"></circle><circle cx="17" cy="19" r="1"></circle></svg>"); addBtn.setAttribute("aria-label", outOfStock?`${p.name}: stokda yoxdur`:(p.customizable?`${p.name} üçün fərdi dizayn aç`:`${p.name} səbətə əlavə et`)); addBtn.onclick=e=>{e.stopPropagation();if(outOfStock)return;p.customizable?ctx.openCustomization(p.id):ctx.add(p.id)}; priceRow.appendChild(addBtn);
