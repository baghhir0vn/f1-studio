...
async function loadAdminProducts(){
    ...
    let adminProductDragId = null;

    function adminProductsAreReorderable(){
        return !(document.getElementById("adminProductSearch")?.value||"").trim();
    }

    function getAdminProductOrder(){
        return [...s.adminProducts].sort((a,b)=>((Number(a.sortOrder)||0)-(Number(b.sortOrder)||0)) || (Number(a.id)-Number(b.id)));
    }

    async function persistAdminProductOrder(){
        const orderIds=getAdminProductOrder().map(p=>Number(p.id));
        if(!orderIds.length)return;
        try{
            const btn=document.getElementById("adminProductOrderSave");
            if(btn)btn.disabled=true;
            await ctx.adminApi(ADMIN_API.productReorder,{method:"POST",body:JSON.stringify({order_ids:orderIds})});
            orderIds.forEach((id,idx)=>{const p=s.adminProducts.find(x=>Number(x.id)===id);if(p)p.sortOrder=(idx+1)*10;});
            showToast("✅ Məhsul sıralaması yadda saxlanıldı.");
        }catch(e){
            console.error("Admin product reorder error:",e);
            showToast(`Sıralama yadda saxlanmadı: ${safeUserError(e,"Sorğu yerinə yetirilmədi.")}`);
            await ctx.loadAdminProducts();
        }finally{
            const btn=document.getElementById("adminProductOrderSave");
    ...
    function moveAdminProduct(id,direction){
        if(!adminProductsAreReorderable()) return showToast("Sıralamaq üçün əvvəlcə axtarışı təmizlə.");
        const list=getAdminProductOrder();
        const idx=list.findIndex(p=>Number(p.id)===Number(id));
        const next=idx+(direction<0?-1:1);
        if(idx<0 || next<0 || next>=list.length)return;
        [list[idx],list[next]]=[list[next],list[idx]];
        list.forEach((p,i)=>p.sortOrder=(i+1)*10);
        s.adminProducts=[...list];
        renderAdminProducts();
        persistAdminProductOrder();
    }

    function handleAdminProductDragStart(e){
        if(!adminProductsAreReorderable()) { e.preventDefault(); return; }
        adminProductDragId=Number(e.currentTarget.dataset.productId);
        e.currentTarget.classList.add("is-dragging");
        try{e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",String(adminProductDragId));}catch(_){}
    }

    function handleAdminProductDragOver(e){
        e.preventDefault();
        if(!adminProductDragId)return;
        const row=e.currentTarget;
        if(Number(row.dataset.productId)===adminProductDragId)return;
        document.querySelectorAll("#adminProductsBody tr.admin-product-sort-row.is-over").forEach(x=>x.classList.remove("is-over"));
        row.classList.add("is-over");
    }

    function handleAdminProductDrop(e){
        e.preventDefault();
        const targetId=Number(e.currentTarget.dataset.productId);
        document.querySelectorAll("#adminProductsBody tr.admin-product-sort-row").forEach(x=>x.classList.remove("is-over","is-dragging"));
        const fromId=adminProductDragId; adminProductDragId=null;
        if(!fromId || !targetId || fromId===targetId)return;
        const list=getAdminProductOrder();
        const from=list.findIndex(p=>Number(p.id)===fromId);
        const to=list.findIndex(p=>Number(p.id)===targetId);
    ...
    function handleAdminProductDragEnd(){
        adminProductDragId=null;
        document.querySelectorAll("#adminProductsBody tr.admin-product-sort-row").forEach(x=>x.classList.remove("is-over","is-dragging"));
    }

    async function setAdminProductFlag(id, flag){
        if(!ctx.isAdminUser?.()) return showToast('Admin girişiniz olmalıdır.');
        const p=s.adminProducts.find(x=>Number(x.id)===Number(id));
        if(!p)return;
        if(flag!=='isActive' && flag!=='isFeatured')return;
        const value=!Boolean(p[flag]);
        try{
            const body={}; body[flag]=value;
            const data=await ctx.adminApi(`${ADMIN_API.products}/${Number(id)}`,{method:'PATCH',body:JSON.stringify(body)});
            const updated=data.product;
            const idx=s.adminProducts.findIndex(x=>Number(x.id)===Number(id));
            if(updated && idx>=0) s.adminProducts[idx]=updated;
            else if(idx>=0) s.adminProducts[idx]={...s.adminProducts[idx],[flag]:value};
            renderAdminProducts();
            if(flag==='isActive') showToast(value?`✅ “${p.name}” saytda aktiv edildi.`:`“${p.name}” saytdan gizlədildi.`);
            else showToast(value?`⭐ “${p.name}” ana səhifə üçün seçildi.`:`“${p.name}” ana səhifə seçilmişlərindən çıxarıldı.`);
            if(typeof ctx.loadServerProducts==='function') await ctx.loadServerProducts();
        }catch(e){
            console.error('Admin product flag error:',e);
            showToast(`Dəyişiklik yadda saxlanmadı: ${safeUserError(e,'Sorğu yerinə yetirilmədi.')}`);
        }
    }
    ...
    function renderAdminProducts(){
        ...
        const ordered=getAdminProductOrder();
        const list=ordered.filter(p=>!q || normalizeText([p.name,p.cat,p.desc,p.material].join(" ")).includes(q));
        const saveBtn=document.getElementById("adminProductOrderSave");
        if(saveBtn)saveBtn.disabled=!reorderable;
        if(!list.length){body.innerHTML='<tr><td colspan="7" class="admin-empty">Məhsul tapılmadı.</td></tr>';return;}
        body.innerHTML=list.map((p,index)=>{
            const qty=p.stockQuantity==null?null:Number(p.stockQuantity);
            const stockLabel=qty==null?escapeHTML(p.stock||"-"):(qty<=0?"Stokda yoxdur":(qty<=5?`<span class="stock-low">Az qalıb: ${qty}</span>`:`${qty} ədəd`));
            const galleryCount=Array.isArray(p.images)&&p.images.length?p.images.length:(p.image?1:0);
            const pos=index+1;
            const dragAttrs=reorderable?`draggable="true" data-product-id="${Number(p.id)}"`:"";
            const sortCell=reorderable ? `<td class="admin-sort-cell"><span class="admin-drag-handle" title="Tutub sürüşdür">⋮⋮</span><b>${pos}</b><div class="admin-sort-buttons"><button type="button" data-action="moveAdminProduct" data-action-args='[${Number(p.id)},-1]' aria-label="Yuxarı" ${pos===1?"disabled":""}>↑</button><button type="button" data-action="moveAdminProduct" data-action-args='[${Number(p.id)},1]' aria-label="Aşağı" ${pos===list.length?"disabled":""}>↓</button></div></td>` : `<td class="admin-sort-cell admin-sort-locked">—</td>`;
            return `<tr class="admin-product-sort-row" ${dragAttrs}><td><div class="admin-product-thumb">${safeResourceUrl(p.image)?`<img loading="lazy" decoding="async" src="${escapeHTML(safeResourceUrl(p.image))}" alt="" style="width:100%;height:100%;object-fit:cover" data-fallback-emoji="${escapeHTML(p.emoji||"📦")}">`:escapeHTML(p.emoji||"📦")}</div></td>${sortCell}<td><b>${escapeHTML(p.name)}</b><div class="admin-muted">${p.isActive===false?'<span class="admin-product-status is-off">● Passiv</span>':'<span class="admin-product-status is-on">● Aktiv</span>'}${p.isFeatured?` · <span class="admin-product-status is-featured">★ Ana səhifə</span>`:''}${escapeHTML(p.badge?` · ${p.badge}`:'')}${galleryCount?` · ${galleryCount} şəkil`:""}</div></td><td>${escapeHTML(p.cat||"")}</td><td>${money(p.price)}</td><td>${stockLabel}</td><td><div class="admin-actions admin-product-actions"><button type="button" class="admin-flag-btn ${p.isActive===false?'is-off':''}" data-action="toggleAdminProductFlag" data-action-args='[${Number(p.id)},&quot;isActive&quot;]'>${p.isActive===false?'● Aktiv et':'● Passiv et'}</button><button type="button" class="admin-flag-btn ${p.isFeatured?'is-featured':''}" data-action="toggleAdminProductFlag" data-action-args='[${Number(p.id)},&quot;isFeatured&quot;]'>${p.isFeatured?'★ Seçilmişdən çıxar':'★ Ana səhifəyə əlavə et'}</button><button type="button" data-action="editAdminProduct" data-action-args='[${Number(p.id)}]'>✏️ Dəyiş</button><button type="button" class="danger" data-action="deleteAdminProduct" data-action-args='[${Number(p.id)}]'>🗑 Sil</button></div></td></tr>`;
        }).join("");
        body.querySelectorAll("img[data-fallback-emoji]").forEach(img=>{
            img.addEventListener("error",()=>{const emoji=img.dataset.fallbackEmoji||"📦"; const parent=img.parentNode; img.remove(); if(parent)parent.textContent=emoji;},{once:true});
        });
        body.querySelectorAll('tr.admin-product-sort-row[draggable="true"]').forEach(row=>{
            row.addEventListener("dragstart",handleAdminProductDragStart);
            row.addEventListener("dragover",handleAdminProductDragOver);
            row.addEventListener("drop",handleAdminProductDrop);
            row.addEventListener("dragend",handleAdminProductDragEnd);
        });
    }
    ...
    function renderAdminGalleryPreview(items=[]){
        ...
        items.slice(0,6).forEach((item,idx)=>{
            ...
            const img=document.createElement("img"); img.src=item.url; img.loading="lazy"; img.decoding="async"; img.alt=`Qalereya şəkli ${idx+1}`; wrap.appendChild(img);
    ...
    function setAdminImagePreview(url, label="Şəkil yoxdur"){
        ...
        if(url){
            const img=document.createElement("img"); img.src=url; img.loading="eager"; img.decoding="async"; img.alt="Məhsul şəkli";
    ...
    async function removeStorageObjects(bucket,urls=[]){
        const paths=urls.map(value=>{
            ...
            if(!raw) return '';
            if(bucket === 'product-images' && raw.startsWith('products/')) return raw;
            if(bucket === 'site-content' && /^(categories|heroes)\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(jpg|jpeg|png|webp|gif|avif)$/i.test(raw)) return raw;
            return ctx.storagePathFromPublicUrl(raw,bucket);
        }).filter(Boolean);
    ...
    function newAdminProduct(){
        ...
        document.getElementById("adminProductCat").value="Lazer & Taxta"; document.getElementById("adminProductCustom").checked=true; document.getElementById("adminProductActive").checked=true; document.getElementById("adminProductFeatured").checked=false;
    ...
    async function saveAdminProduct(e){
        ...
        try{
            ...
            const friendly={
                ...
                "new row violates row-level security policy":"Storage policy icazə vermir. Storage V2 SQL-i tətbiq edin.","column products.is_active does not exist":"V109 məhsul statusu SQL-i hələ tətbiq edilməyib.","column products.is_featured does not exist":"V109 məhsul statusu SQL-i hələ tətbiq edilməyib."
            };
...
export function initProducts(){
  Object.assign(ctx, {
    loadAdminProducts, renderAdminProducts, moveAdminProduct, persistAdminProductOrder, setAdminProductFlag, toggleAdminProductFlag: setAdminProductFlag, clearAdminGalleryPreviewUrls, renderAdminGalleryPreview, revokeAdminImagePreview, setAdminImagePreview, handleAdminProductImage, clearAdminProductImage, storagePathFromPublicUrl, removeStorageObjects, uploadAdminProductImage, resetAdminProductImageUI, newAdminProduct, editAdminProduct, cancelAdminProduct, collectAdminProduct, saveAdminProduct, deleteAdminProduct
  });
}
