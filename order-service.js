...
export const orderService = {
    ...
    async listAdmin() {
        const { data, error } = await sb.rpc('admin_list_orders', { p_limit: 5000 });
        if (error) throw error;
    ...
    async setStatus(id, status) {
        ...
        return data && typeof data === 'object'
            ? data
            : { id: Number(id), status: String(status), stock_restored: false };
    },
    async setAdminNote(id, note) {
        if (!Number.isInteger(Number(id)) || Number(id) <= 0) throw new Error('INVALID_ORDER');
        const clean = String(note || '').trim().slice(0, 2000);
        const { error } = await sb.from('orders').update({ admin_note: clean }).eq('id', id);
        if (error) throw error;
        return { id: Number(id), adminNote: clean };
    }
};
